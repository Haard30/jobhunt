            ...
            await sails.sendNativeQuery(sqlSelect, async function (err, results) {
                ...
                if (err) {
                    ...
                    if (length < 1) {
                        ...
                        rtype = results["rows"][0]["registration_type"];
                        req.session.address = results["rows"][0]["address"];
                        res.view('pages/twoFactorQuestions', { email, question, answer, name, rtype });
                    }
                }
            });
    ...
    authenticateanswer: async function (req, res) {
        if(questionNumber === 0 ){
            ...
            if(userAnswer.toLowerCase() === correctAnswer.toLowerCase()){
                ...
                req.session.rtype = req.body.rtype;
        ...
        await sails.sendNativeQuery(sqlSelect, async function (err, results) {
            ...
            if (err) {
                ...
                if (length < 1) {
                    ...
                    let vals = "values('"+email+"' , "+" '"+req.body.txtpassword+"' , "+" '"+req.body.txtfname+"' , "+" '"+req.body.lname+"' , "+" '"+req.body.txtquetion1+"' , "+" '"+req.body.txtanswer1+"' , "+" '"+req.body.txtquetion2+"' , "+" '"+req.body.txtanswer2+"' , "+" '"+req.body.rtype+"')"
    ...
    goHome: async function (req, res) {
        if(req.session.authenticated != true ){
            ...
            if(req.session.rtype === "applicant"){
                ...
                res.view('pages/applicantHome', { name});
            }
            else{   
            res.view('pages/employerHome', { name});
            }
        }
